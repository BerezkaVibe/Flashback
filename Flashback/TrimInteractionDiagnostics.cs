...
internal static class TrimInteractionDiagnostics
{
    internal static async Task RunAsync()
    {
        ...
        try
        {
            ...
            cutAdded.Invoke(trim,new object[]{new CutRegion(-1,10,10.5)});
            ...
            tl.PickTime!(10.5);
            Check(tl.PickTime==null && tl.ZoomRegions.Any(r=>Math.Abs(r.Start-10.5)<1e-9 && Math.Abs(r.End-11.8)<1e-9),"Picking on the timeline moves the start, right up against a cut-out"+(tl.ZoomRegions.Any(r=>Math.Abs(r.Start-10.5)<1e-9) ? "" : $" [status: {trim.StatusLabel.Text}; zooms: {string.Join(",",tl.ZoomRegions.Select(r=>$"{r.Start}-{r.End}"))}; cuts: {string.Join(",",tl.Cuts.Select(c=>$"{c.Lane}:{c.Start}-{c.End}"))}; overlays {tl.Overlays.Count}]"));
            trim.SeekTo(4); tl.ZoomMode=true;
            double snapped=(double)typeof(TrimTimeline).GetMethod("CutTimeAt",flags)!.Invoke(tl,new object[]{tl.XAt(10)+2})!;
            tl.ZoomMode=false;
            Check(Math.Abs(snapped-10)<1e-9,"Placing parts snaps to a cut-out's edge");
            var zoomNow=tl.ZoomRegions.First(r=>Math.Abs(r.Start-10.5)<1e-9);
            Raise("PartResizeStarted",zoomNow); Raise("PartResized",zoomNow,10.5,11.4); Raise("PartResized",zoomNow,10.5,11.9); Raise("PartResizeFinished");
            Check(tl.ZoomRegions.Any(r=>Math.Abs(r.Start-10.5)<1e-9 && Math.Abs(r.End-11.9)<1e-9),"Dragging a zoom's end changes its length");
            Raise("PartResizeStarted",tl.ZoomRegions.First(r=>Math.Abs(r.Start-10.5)<1e-9)); Raise("PartResized",zoomNow,9.5,11.9); Raise("PartResizeFinished");
            Check(tl.ZoomRegions.Any(r=>Math.Abs(r.Start-10.5)<1e-9),"Dragging stops at a video cut-out instead of overlapping it");
            trim.HandleKey(Key.Z,ModifierKeys.Control,false);
            ...
            // Stacked parts: clicking the same spot again steps to the part underneath, and back round.
            typeof(TrimWindow).GetMethod("SlowAdded",flags)!.Invoke(trim,new object[]{10.5,11.8});
            trim.UpdateLayout();
            var partAt=typeof(TrimTimeline).GetMethod("PartAt",flags)!; var spot=new Point(tl.XAt(11),30);
            tl.FocusedPart=null; var firstPick=partAt.Invoke(tl,new object[]{spot}); tl.FocusedPart=firstPick;
